...
BASE_DIR     = os.path.dirname(os.path.abspath(__file__))   # src/ (local) or repo root (cloud)
PROJECT_ROOT = os.path.dirname(BASE_DIR)                      # ML_project/ (local) or parent (cloud)

# DATA: look next to the script first, then in data/ subfolder (local structure)
_data_local  = os.path.join(BASE_DIR, "Student Performance Analytics Dataset.csv")
_data_subdir = os.path.join(BASE_DIR, "data", "Student Performance Analytics Dataset.csv")
_data_parent = os.path.join(PROJECT_ROOT, "data", "Student Performance Analytics Dataset.csv")
DATA_PATH = _data_local if os.path.exists(_data_local) else \
            _data_subdir if os.path.exists(_data_subdir) else _data_parent

# MODEL: use /tmp on Streamlit Cloud (read-only repo), local src/ otherwise
MODEL_PATH   = os.path.join(
    "/tmp" if not os.access(BASE_DIR, os.W_OK) else BASE_DIR,
    "model.pkl"
)
